...
46 themes, each a self-contained token file in `src/styles/themes/<slug>.css`, scoped under `.theme-<slug>`.
DNA themes carry real crawled values from the source products (colors, radii, closest Google Font).
shadcn semantic vars are bridged to theme tokens in `src/styles.css`; every theme has a unique font identity.
