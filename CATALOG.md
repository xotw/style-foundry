...
73 themes (30 aesthetics, 43 product DNA), one token file each in `src/styles/themes/<slug>.css`, scoped `.theme-<slug>`.
DNA themes carry real crawled values from the source products. Every theme has a unique font identity.
System layer: per-theme design rules in `src/lib/style-rules.ts`. Registry: `npx shadcn@latest add <site>/r/<slug>.json`.
Live: `/styles/<slug>` (landing + rules), `/components`, `/blocks`, `/app` (interactive demo).
...
- **shadcn CLI**: `npx shadcn@latest add https://style-foundry-gabbulldo.vercel.app/r/<slug>.json` — or register `@style-foundry` in components.json.
- **Direct**: copy `src/styles/themes/<slug>.css` + wrap your app in `class="theme-<slug>"`.
- **Lovable**: paste the theme file into your first prompt — "use these exact design tokens".
